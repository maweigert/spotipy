...
def multiscale_unet(
    ...
    for n in range(n_depth):
        for i in range(n_conv_per_depth):
            layer = conv_block(int(n_filter_base * expansion_factor ** n), kernel_size, kernel_size,
                                   dropout=dropout,
    ...
    for i in range(n_conv_per_depth - 1):
        layer = conv_block(int(n_filter_base * expansion_factor ** n_depth), kernel_size, kernel_size,
                               dropout=dropout,
                               init=kernel_init,
                               activation=activation,
                               batch_norm=batch_norm, name=_name("middle_%s" % i))(layer)

    layer = conv_block(int(n_filter_base * expansion_factor ** max(0, n_depth - 1)), kernel_size, kernel_size,
                           dropout=dropout,
                           activation=activation,
                           init=kernel_init,
    ...
    for n in reversed(range(n_depth)):
        ...
        multi = layer 
        for i in range(n_conv_per_depth):
            multi = conv_block(int(n_filter_base * expansion_factor ** n), kernel_size, kernel_size,
                                   dropout=dropout,
                                   init=kernel_init,
                                   activation=activation,
        ...
        layer = conv_block(int(n_filter_base * expansion_factor ** max(0, n - 1)), kernel_size, kernel_size,
                               dropout=dropout,
                               init=kernel_init,
                               activation=activation,
    ...
    # final_flow = tf.keras.layers.Lambda(lambda x: tf.keras.backend.l2_normalize(x, axis=-1))(final_flow)
